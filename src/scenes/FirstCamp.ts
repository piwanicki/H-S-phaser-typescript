...
export default class FirstCamp extends Phaser.Scene {
    ...
    create() {
        ...
        const map = this.make.tilemap({ key: "firstCamp" });
        const forestTileset = map.addTilesetImage('forest', 'forest', 32, 32, 1,2);
        const graveyardTileset = map.addTilesetImage('grtaveyard','graveyard', 32, 32, 0,0);
        this.ground = map.createDynamicLayer("ground", forestTileset, 0, 0);
        this.water = map.createDynamicLayer('water', forestTileset, 0, 0);
        this.paths = map.createDynamicLayer('paths', forestTileset, 0, 0);
        this.grass = map.createDynamicLayer('grass', forestTileset, 0, 0);
        this.grass = map.createDynamicLayer('stones', forestTileset, 0, 0);
        this.trees = map.createDynamicLayer('trees', forestTileset, 0, 0);
        this.trees = map.createDynamicLayer('buildings', graveyardTileset, 0, 0);
        //this.enters = map.createStaticLayer('enters', forestTileset, 0, 0);
        ...
        // trees layer
        //const trees = map.createStaticLayer('trees', forestTileset, 0, 0)
